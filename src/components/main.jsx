...
export default function Main(){
    ...
    const ing=ingredients.map((ingredient)=>{
        return <li className="ingredients-list" key={ingredient}>{ingredient}</li>
    })
    const [recipeShown,setRecipeShown]=useState(false)

    function recipeShow(){
    setRecipeShown(prevSetRecipe=>true)
    }
    return(
        <main>
            <form action={formsubmission} className="Add-ingredient-form">
            ...
            {ingredients.length> 0 && <section>
                ...
                {ingredients.length>4 &&<div className="get-recipe-container">
                    ...
                    <button onClick={recipeShow}>Get a Recipe</button>
                </div>}
